// Anti-Cheating & Student Exam Engine - CBT Kusuma
...
function startExamWithFullscreen() {
  ...
  // Initialize Event Listeners for Cheating Detection
...
function initCheatingProtection() {
  // 1. Visibility API (Tab Switch / App Switching)
  document.addEventListener('visibilitychange', () => {
    if (document.hidden && isExamStarted && currentSession?.status === 'IN_PROGRESS') {
      triggerViolation('TAB_SWITCH', 'Pindah tab atau berpindah ke aplikasi lain');
    }
  });
  ...
  // 4. Disable Right Click & Keyboard Shortcuts (Copy/Paste/DevTools)
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (
      ...
      e.preventDefault();
      triggerViolation('DEVTOOLS', 'Mencoba membuka DevTools atau shortcut keyboard dilarang');
    }
  });
}
...
async function submitExam() {
  if (confirm('Apakah Anda yakin ingin mengumpulkan ujian? Sesi akan diakhiri.')) {
    try {
